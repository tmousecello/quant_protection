...
3. **但多數決的保護在「複本同速累積」下是一次性的**:首次多數決失敗
   (≥2 副本同 bit 損壞)後,因「拒絕服務錯值 → 不回寫 → 副本永不再同步」,防護
   **永久失效**,recall 沿無防護軌跡崩至 0.0。多數決需要搭配「副本自身的定期驗證
   重載」(或 ECC)才不是一次性保險絲。(註:本點的 tick-2 首失敗為舊 XOR lane 單
   seed 抽樣;分布層結論由 Stage 3 #2 新 lane 取代 —— N=30,median 21、IQR [9,30]、
   min 0;「一次性保險絲」的機制敘述不受 lane 影響。)
...
- 成本:R=3 副本 = 192 B 記憶體(gross,3 份;論文取**增量口徑** 132 B = 2 份額外
  複本 128 B + 4 B CRC —— 兩者僅是同一事的不同計法,勿混用)(vs 索引 280 MB,佔
  7×10⁻⁷)+ 每 tick 512-bit 逐 bit 投票 + 一次 64 B CRC。這是 F3 成本模型
  (mem_cost + scrub_overhead)裡幾乎免費的一側 —— 「保護 64 B 就擋掉最大的災難」
  是量化索引防護的第一優先。
...
## 5. 複本同時累積:多數決的失效邊界(run 4,舊 XOR lane)

> **審計標註(第二輪誠實性審計)**:本節數字為舊 XOR lane(aliasing-prone);分布層
> 結論已由 Stage 3 #2(新 SeedSequence lane,N=30:median 21、IQR [9, 30]、min 0)取代。
> 本節保留為機制發現的歷史記錄 ——「多數決是一次性保險絲」的機制敘述不受 lane 影響
> (新 lane 兩條極端 seed 的完整 timeline 重現同一機制)。以下 tick 值(含 tick 2 首失敗)
> 為單 seed 舊 lane 抽樣,不可與新 lane 逐值比對。
...
- **機制**:多數決失敗(某 bit 有 ≥2/3 副本同時錯)→ 依「不服務錯值」語意拒絕回寫
  ...
  **單次瞬時雙擊 = 永久解除防護**;這個設計是誠實的(絕不拿錯的 majority 蓋掉資料),
  但也是一次性的保險絲。
- **統計注**:tick 2 即失敗是偏早的單 seed 抽樣(且受舊 lane aliasing 加成;新 lane
  分布見 Stage 3)(每 tick 失敗機率隨副本淨損上升,以 tick 2 的 [1,3,5] bits 估計
  單 tick 約 4–5%,期望首失敗在 O(10) ticks 級(Stage 3 實測 median 21,確認));
  但「短命」的結論對這個量級不敏感 —— 同速累積下 R=3 多數決撐不過幾十個 tick。
  精確首失敗分布已由 Stage 3 #2 補齊(N=30,median 21、IQR [9, 30])。
...
F3 的敘事鏈因此完整:**懸崖端**用近乎免費的 132 B(增量口徑)複本把「<1 tick 靜默全崩」變成
...
- ✚ 數據最戲劇且本輪全部實測完備:6 bits(佔索引 2.7×10⁻⁶%)tick-0 靜默全崩、132 B 修復使防護可再生、複本不 scrub 為一次性保險絲(首失敗 median ~21 ticks,N=30)——自洽閉環。
...
**危險度不對稱(6 個數量級)→ 懸崖用 132 B 修掉但複製≠安全(不 scrub 為一次性保險絲,
首失敗 median ~21 ticks)→ 斜坡用 CRC+EB 偵測恢復、EB 免費優於 drop → 防護預算應按
危險度分配,而非按位元組數分配。**
...
2. run 4 的複本 seed lane 沿用 `seed ^ tick ^ ((r+1)·0x5EED)`;expb 曾在類似 XOR lane
   ...
   **(stage 3 已遷移)**:複本 lane 改為 `SeedSequence([root, tick, r+1])` 的 64-bit 輸出
   (`phase3_e5_recovery.replica_lane_seed`;64-bit 使 30×150×3=13,500 lane 的生日碰撞率
   由 32-bit 的 ~2% 降到 ~5e-12),主 buf lane 不變;root 1234 的
